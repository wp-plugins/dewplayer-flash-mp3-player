...
/*
	if(strpos($the_content, "[dewplayer:")!==FALSE) {
		...
		preg_match_all('/\[(?<name>\w+):([)a-zA-Z0-9\/:\.]+)/', $the_content, $matches, PREG_SET_ORDER); 
		foreach($matches as $match) { 
		$the_content = preg_replace("/\[(?<name>\w+):([)a-zA-Z0-9\/:\.]+)\]/", dewplayer_tag($match[2]), $the_content,1);
		}
...
*/
	if(strpos($the_content, "[dewplayer:")!==FALSE) {
		...
		preg_match_all('/\[dewplayer:([)a-zA-Z0-9\/:\.\|]+)/', $the_content, $matches, PREG_SET_ORDER); 
		foreach($matches as $match) { 
		$the_content = preg_replace("/\[dewplayer:([)a-zA-Z0-9\/:\.\|]+)\]/", dewplayer_tag($match[1]), $the_content,1);
		}
