...
function dewplayer_check($the_content) {
	if(strpos($the_content, "[dewplayer:")!==FALSE) {
		...
		preg_match_all('/\[dewplayer:([)a-zA-Z0-9\/:\.\|\-_]+)/', $the_content, $matches, PREG_SET_ORDER); 
		foreach($matches as $match) { 
		$the_content = preg_replace("/\[dewplayer:([)a-zA-Z0-9\/:\.\|\-_]+)\]/", dewplayer_tag($match[1]), $the_content,1);
		}
